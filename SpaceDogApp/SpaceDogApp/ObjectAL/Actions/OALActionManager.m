...
- (id) init
{
	if(nil != (self = [super init]))
	{
      ...
      // Updated, as targets may reasonably reach retainCount
      //  zero when owners are done, but have no means to be
      //  removed from this array when that happens.
		targets = [[NSMutableArray alloc] initWithCapacity:50];
...
- (void) dealloc
{
	...
	Release(targets);
	Release(targetActions);
	Release(actionsToAdd);
	Release(actionsToRemove);
	[super dealloc];
}
